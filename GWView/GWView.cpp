...
GW3dView::GW3dView(const GWDocument& d)
    ...
    , _root(new osg::Group)
{
    _nodeMap.insert(d.getSceneNodeId(),_root);
}
...
void GW3dView::activateTrackballMode()
{
    //_osgViewer->setCameraManipulator(new osgGA::TrackballManipulator());

}

void GW3dView::activateSelectMode()
{
    //_osgViewer->setCameraManipulator(0);

}

void GW3dView::activateManipulateMode()
{
    //_osgViewer->setCameraManipulator(0);
...
QMap<int,osg::ref_ptr<osg::Node> >& GW3dView::getNodeMap()
{
    return _nodeMap;
}
